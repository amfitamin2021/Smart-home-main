...
public class ThingsBoardIntegrationService {
    ...
    /**
     * Получает историю телеметрии устройства из ThingsBoard
     * @param device Устройство
     * @param key Ключ телеметрии (например, "humidity")
     * @param startTs Начальная метка времени в миллисекундах
     * @param endTs Конечная метка времени в миллисекундах
     * @return Список значений телеметрии с метками времени
     */
    public List<Map<String, Object>> getDeviceTelemetryHistory(Device device, String key, long startTs, long endTs) {
        if (!ensureAuthenticated() || device.getThingsboardToken() == null) {
            return new ArrayList<>();
        }
        
        try {
            String url = thingsBoardUrl + "/api/plugins/telemetry/DEVICE/" + device.getThingsboardDeviceId() + 
                    "/values/timeseries?keys=" + key + "&startTs=" + startTs + "&endTs=" + endTs;
            ...
            log.warn("Не удалось получить историю телеметрии для устройства {}. Код: {}, Тело: {}", 
                    device.getName(), response.getStatusCode(), response.getBody());
            
        } catch (Exception e) {
            log.error("Ошибка при получении истории телеметрии для устройства {}: {}", 
                    device.getName(), e.getMessage(), e);
        }
